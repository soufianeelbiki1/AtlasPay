...
Nexus needs an operational view of AtlasPay without reaching directly into AtlasPay's database, mutating payment state, or presenting fixture values as live telemetry. AtlasPay currently has durable PostgreSQL state for payments, ledger/reconciliation, and the transactional outbox. Network observations are persisted as low-cardinality operational metadata. They deliberately exclude PAN, STAN, RRN, DE55 and message payloads, while retaining route, issuer, acquirer, outcome, disposition, delivery-unknown state and elapsed time.
...
- derives network totals, dispositions, timeout/late-response counts, p95 latency and route/issuer/acquirer breakdowns from durable observations;
...
Nexus can consume a stable API instead of sharing AtlasPay persistence concerns. Consumers must handle partial/unavailable sections explicitly. Route metrics are aggregate operational facts, not per-transaction traces; consumers must not infer unavailable transaction details or treat a timeout as an issuer decline.
...
The additive route breakdown preserves the existing zero/unknown semantics. A breaking schema change still requires a new contract version.
...
- the real PostgreSQL aggregation queries, including route-level timeout and delivery-unknown facts, against PostgreSQL 16;
